...
class ShareAPIFacade:
    ...
    def create(self, endpoint, file_path, opaque_id, idp, role=Role.EDITOR, grantee_type=Grantee.USER, reshare=True):
        if self._is_ocm_user(opaque_id, idp):
            self.ocm_share_api.create(opaque_id, idp, idp, endpoint, file_path, grantee_type, role, reshare)
        else:
            self.share_api.create(endpoint, file_path, opaque_id, idp, role, grantee_type)
    ...
    def map_shares_to_model(self, list_response, received=False):
        ...
        for share in list_response.shares:
            if received:
                share = share.share
            # todo: fix reading unacepted share files
            try:
                ...
                model = {'name': share.resource_id.opaque_id.rsplit('/', 1)[-1],
                         'path': share.resource_id.storage_id + ':' + share.resource_id.opaque_id,
                         'last_modified': '',
                         'created': '',
